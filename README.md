...
La arquitectura de percepción local conserva la interpolación en unidades métricas y la posterior fusión sensorial mediante un Filtro de Media Móvil Exponencial (EMA) y un Filtro de Kalman escalar 1D. El modelo de predicción cinemática ($\hat{x}_k = \hat{x}_{k-1} - \Delta s$) permite amortiguar los picos de ruido causados por la dispersión infrarroja en superficies diagonales. Esta señal filtrada dicta el cruce hacia la lógica de evasión reactiva ante cualquier objeto detectado a menos de $0.025\text{ m}$ (Umbral de Seguridad), evitando falsos gatillos durante la navegación en el espacio libre.
